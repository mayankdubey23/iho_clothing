...
export default function Shop({ filters = {}, products, categories = [], brands = [], colors = [], sizes = [], cms = {} }) {
    ...
    const colorOptions = [
        ...colors.map((color) => ({
            ...
            name: color.name,
            hex_code: color.hex_code || '#ffe1e8',
        })),
    ...
    const FilterSidebar = () => (
        <div className="flex w-full flex-col gap-8">
            <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                <span className="text-xs font-black uppercase tracking-[0.22em] text-[#282c3f]">Filters</span>
                <Link href="/shop" className="text-[9px] font-black uppercase tracking-[0.2em] text-[#ff3f6c] hover:text-[#282c3f]">
                    Clear All
                </Link>
            </div>

            <div className="border-b border-slate-200 pb-6">
                <button onClick={() => toggleSection('gender')} className="mb-4 flex w-full items-center justify-between text-xs font-black uppercase tracking-widest text-[#282c3f]">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {['men', 'women', 'unisex'].map((gender) => (
                                    <button
                                        ...
                                        className={`border px-4 py-3 text-[10px] font-black uppercase tracking-widest transition-colors ${filters?.gender === gender ? 'border-[#282c3f] bg-[#282c3f] text-white' : 'border-slate-200 bg-slate-50 text-[#282c3f] hover:border-[#282c3f]'}`}
                                    >
                                        {gender}
                                    </button>
                                ))}
            ...
            <div className="border-b border-slate-200 pb-6">
                <button onClick={() => toggleSection('categories')} className="mb-4 flex w-full items-center justify-between text-xs font-black uppercase tracking-widest text-[#282c3f]">
                    ...
                    {openFilterSections.categories && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <ul className="flex flex-col gap-4 pl-1 text-[11px] font-bold uppercase tracking-widest text-slate-500">
                                <li>
                                    <Link href={filterUrl({ category: '', subcategory: '' })} className={`transition-colors hover:text-[#282c3f] ${!filters?.category ? 'border-l-2 border-[#ff3f6c] pl-2 font-black text-[#282c3f]' : ''}`}>
                                        All Gear
                                    </Link>
                                </li>
                                ...
                                {categories.map((cat) => {
                                    ...
                                    return (
                                        <li key={cat.id} className="flex flex-col gap-2">
                                            <Link href={filterUrl({ category: cat.slug, subcategory: '' })} className={`transition-colors hover:text-[#282c3f] ${isParentActive ? 'font-black text-[#282c3f]' : ''}`}>
                                                {cat.name}
                                            </Link>
                                                    ...
                                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                                        <ul className="ml-2 mt-2 flex flex-col gap-3 border-l-2 border-slate-100 pl-3">
                                                            {cat.children.map((subCat) => (
                                                                <li key={subCat.id}>
                                                                    <Link href={filterUrl({ category: subCat.slug, subcategory: '' })} className={`text-[10px] transition-colors hover:text-[#282c3f] ${activeCategory?.id === subCat.id ? 'font-black text-[#ff3f6c]' : 'text-slate-400'}`}>
                                                                        {subCat.name}
                                                                    </Link>
                                                                </li>
                                                            ))}
            ...
            {brands.length > 0 && (
                <div className="border-b border-slate-200 pb-6">
                    <button onClick={() => toggleSection('brands')} className="mb-4 flex w-full items-center justify-between text-xs font-black uppercase tracking-widest text-[#282c3f]">
                                ...
                                <div className="flex flex-col gap-3 text-[11px] font-bold uppercase tracking-widest text-slate-500">
                                    {brands.map((brand) => (
                                        <button
                                            ...
                                            className={`text-left transition-colors hover:text-[#282c3f] ${filters?.brand === brand.slug || filters?.brand === brand.name ? 'border-l-2 border-[#ff3f6c] pl-2 font-black text-[#282c3f]' : ''}`}
                                        >
                                            {brand.name}
                                        </button>
                                    ))}
            ...
            <div className="border-b border-slate-200 pb-6">
                <button onClick={() => toggleSection('price')} className="mb-4 flex w-full items-center justify-between text-xs font-black uppercase tracking-widest text-[#282c3f]">
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <div className="grid grid-cols-2 gap-2">
                                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">
                                    Min
                                    <input type="number" min="0" placeholder="₹100" value={priceDraft.min_price} onChange={(e) => setPriceDraft((prev) => ({ ...prev, min_price: e.target.value }))} className="mt-2 w-full border border-slate-200 bg-slate-50 px-3 py-3 text-xs font-black text-[#282c3f] outline-none focus:border-[#282c3f]" />
                                </label>
                                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">
                                    Max
                                    <input type="number" min="0" placeholder="₹600" value={priceDraft.max_price} onChange={(e) => setPriceDraft((prev) => ({ ...prev, max_price: e.target.value }))} className="mt-2 w-full border border-slate-200 bg-slate-50 px-3 py-3 text-xs font-black text-[#282c3f] outline-none focus:border-[#282c3f]" />
                                </label>
                            </div>
                            <button type="button" onClick={() => applyFilter(priceDraft)} className="mt-3 w-full bg-[#282c3f] py-3 text-[9px] font-black uppercase tracking-widest text-white hover:bg-[#ff3f6c]">
                                Apply ₹100 - ₹600
                            </button>
                        </motion.div>
                    )}
            ...
            {colorOptions.length > 0 && (
                <div className="border-b border-slate-200 pb-6">
                    <button onClick={() => toggleSection('colors')} className="mb-4 flex w-full items-center justify-between text-xs font-black uppercase tracking-widest text-[#282c3f]">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {colorOptions.map((color) => (
                                        <button
                                            ...
                                            className={`flex items-center gap-2 text-left text-[10px] font-bold uppercase tracking-widest text-slate-500 transition-colors hover:text-[#282c3f] ${filters?.color === color.name ? 'font-black text-[#282c3f]' : ''}`}
                                        >
                                            <span
                                                className={`h-4 w-4 shrink-0 rounded-full border border-slate-300 ${filters?.color === color.name ? 'ring-2 ring-[#282c3f]/20' : ''}`}
            ...
            {sizeOptions.length > 0 && (
                <div className="border-b border-slate-200 pb-6">
                    <button onClick={() => toggleSection('sizes')} className="mb-4 flex w-full items-center justify-between text-xs font-black uppercase tracking-widest text-[#282c3f]">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {sizeOptions.map((size) => (
                                        <button
                                            ...
                                            className={`border py-2 text-[10px] font-black uppercase tracking-widest transition-colors ${filters?.size === size.code ? 'border-[#282c3f] bg-[#282c3f] text-white' : 'border-slate-200 bg-slate-50 text-[#282c3f] hover:border-[#282c3f]'}`}
                                        >
                                            {size.code}
                                        </button>
                                    ))}
            ...
            <div className="border-b border-slate-200 pb-6">
                <button onClick={() => toggleSection('discount')} className="mb-4 flex w-full items-center justify-between text-xs font-black uppercase tracking-widest text-[#282c3f]">
                            ...
                            <div className="flex flex-col gap-3">
                                {discountRanges.map((discount) => (
                                    <button
                                        ...
                                        className={`text-left text-[11px] font-bold uppercase tracking-widest transition-colors hover:text-[#282c3f] ${Number(filters?.discount) === discount ? 'border-l-2 border-[#ff3f6c] pl-2 font-black text-[#282c3f]' : 'text-slate-500'}`}
                                    >
                                        {discount}% and above
                                    </button>
                                ))}
    ...
    return (
        <AppLayout>
            ...
            {cms?.shop_promo_banner && (
                <div className="bg-[#ff3f6c] px-4 py-2.5 text-center text-[10px] font-black uppercase tracking-[0.2em] text-white">
                    {cms.shop_promo_banner}
                </div>
            )}

            <div className="relative flex items-center justify-center overflow-hidden border-b border-slate-800 bg-[#282c3f] py-32">
                <img src={headerImage} alt={heroTitle} className="absolute inset-0 h-full w-full object-cover object-top opacity-45" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#282c3f] via-[#282c3f]/50 to-[#282c3f]/20" />
            ...
            <div className="mx-auto min-h-screen max-w-[1400px] px-6 py-12 lg:px-8">
                <div className="mb-12 flex flex-col justify-between gap-6 border-b border-slate-200 pb-6 md:flex-row md:items-end">
                    <nav className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
                        <Link href="/" className="transition-colors hover:text-[#282c3f]">Studio</Link>
                        <span>/</span>
                        <span className="text-[#282c3f]">{activeCategory?.name || 'Collection'}</span>
                    </nav>

                    <div className="flex items-center gap-4">
                        <button onClick={() => setIsMobileFilterOpen(true)} className="flex items-center gap-2 border border-slate-200 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-[#282c3f] lg:hidden">
                            <Filter size={14} /> Filters
                        </button>
                        <select value={filters?.sort || ''} onChange={(e) => applyFilter({ sort: e.target.value })} className="border border-slate-200 bg-slate-50 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-[#282c3f] outline-none">
                ...
                <div className="flex flex-col gap-12 lg:flex-row">
                    <div className="sticky top-32 hidden h-fit w-64 shrink-0 lg:block">
                        <div className="mb-8 flex items-center gap-3 text-[#282c3f]">
                    ...
                    <div className="flex-1">
                        {displayProducts.length > 0 ? (
                            <div className="grid grid-cols-1 gap-x-6 gap-y-12 sm:grid-cols-2 xl:grid-cols-4">
                            ...
                            <div className="rounded-2xl border border-dashed border-slate-300 py-24 text-center">
                                <p className="mb-2 text-lg font-black uppercase tracking-tight text-[#282c3f]">No Styles Found</p>
            ...
            <AnimatePresence>
                {isMobileFilterOpen && (
                    <>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-[#282c3f]/80 backdrop-blur-sm lg:hidden" onClick={() => setIsMobileFilterOpen(false)} />
                        <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} className="fixed inset-y-0 left-0 z-[70] flex w-[85%] max-w-sm flex-col bg-white shadow-2xl lg:hidden">
                            ...
                            <div className="flex gap-4 border-t bg-slate-50 p-6">
                                <button onClick={() => setIsMobileFilterOpen(false)} className="flex-1 bg-[#282c3f] py-4 text-[10px] font-black uppercase text-white">
                                    Done
                                </button>
                            </div>
                        </motion.div>
