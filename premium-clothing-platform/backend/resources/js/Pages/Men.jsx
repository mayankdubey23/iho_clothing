...
export default function Men({ categories = [], featuredProducts = [], offer = null, cms = {} }) {
    ...
    return (
        <AppLayout>
            ...
            <section className="relative min-h-[calc(100vh-6rem)] overflow-hidden bg-[#282c3f] text-white">
                <img src={heroImage} alt="Men sportswear" className="absolute inset-0 h-full w-full object-cover opacity-40 grayscale-[20%]" />
                <div className="absolute inset-0 bg-gradient-to-r from-[#282c3f] via-[#282c3f]/78 to-transparent" />
                <div className="relative z-10 mx-auto grid min-h-[calc(100vh-6rem)] max-w-[1400px] grid-cols-1 items-center px-6 py-20 lg:grid-cols-12">
                    ...
                    >
                        <div className="mb-5 flex items-center gap-3">
                            <span className="grid size-11 place-items-center border border-white/20 bg-white text-[#282c3f]"><Dumbbell size={19} /></span>
                        ...
                        <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                            <Link href="/shop?gender=men" className="group inline-flex items-center justify-center gap-3 bg-white px-8 py-4 text-[10px] font-black uppercase tracking-[0.3em] text-[#282c3f] transition-colors hover:bg-[#ff3f6c] hover:text-white">
            ...
            <section className="bg-[#f5f5f6] py-20">
                <div className="mx-auto max-w-[1400px] px-6">
                    <div className="mb-10 flex flex-col justify-between gap-5 border-b border-slate-200 pb-6 md:flex-row md:items-end">
                        ...
                        <Link href="/shop?gender=men" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.25em] text-[#282c3f] hover:text-[#ff3f6c]">
                    ...
                    {products.length > 0 ? (
                        ...
                        <div className="border border-dashed border-slate-300 bg-white py-20 text-center">
                            ...
                            <p className="text-lg font-black uppercase italic tracking-tight text-[#282c3f]">No active men products yet</p>
            ...
            {(offer || cms.men_offer_text) && (
                <section className="bg-white py-16">
                    <div className="mx-auto max-w-[1400px] px-6">
                        <Link href="/shop?gender=men&offer=active" className="group flex flex-col justify-between gap-8 bg-[#282c3f] p-8 text-white transition-colors hover:bg-black md:flex-row md:items-center md:p-12">
                            <div>
...
function HeroMetric({ icon: Icon, label }) {
    return (
        <div className="border border-white/15 bg-white/10 p-5 backdrop-blur">
            <Icon size={22} className="mb-5 text-[#ff3f6c]" />
...
function SectionHeader({ eyebrow, title }) {
    return (
        <div>
            <p className="mb-3 text-[10px] font-black uppercase tracking-[0.35em] text-[#ff3f6c]">{eyebrow}</p>
            <h2 className="text-4xl font-black uppercase italic tracking-tight text-[#282c3f]">{title}</h2>
        </div>
    );
}
