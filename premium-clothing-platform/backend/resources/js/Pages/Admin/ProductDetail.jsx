                ...
                <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="aspect-[4/5] w-full bg-slate-50 overflow-hidden relative">
                            {product.badge && (
                                <div className="absolute left-6 top-6 z-20 bg-[#E94E3C] px-4 py-1.5 text-[10px] font-black uppercase tracking-widest text-white">
                                    {product.badge}
                                </div>
                            )}
                            {product.image_path ? (
                        ...
                        {product.gallery?.length > 0 && (
                            <div className="grid grid-cols-4 gap-4">
                                {product.gallery.map(img => (
                                    <div key={img.id} className="aspect-square bg-slate-50 cursor-pointer border-2 border-transparent hover:border-[#1A1A2E]">
                    ...
                    <div className="flex flex-col py-6 lg:pl-8">
                        {product.category?.name && (
                            <p className="mb-3 text-[10px] font-black uppercase tracking-[0.3em] text-[#E94E3C]">
                                {product.category.name}
                            </p>
                        )}
                        <h1 className="mb-4 text-3xl font-black uppercase tracking-tighter text-[#1A1A2E] lg:text-5xl">
                            {product.name}
                        </h1>

                        <div className="mb-6 flex items-center gap-4">
                            <span className="text-2xl font-black text-[#1A1A2E]">₹{product.price}</span>
                            {hasDiscount && (
                                <>
                        ...
                        {product.available_colors?.length > 0 && (
                            <div className="mb-8">
                                <div className="mb-3 flex items-center justify-between">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-[#1A1A2E]">Color</span>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {product.available_colors.map(color => (
                                        <button
                                            ...
                                            className={`size-10 rounded-none border-2 transition-transform ${selectedColor?.id === color.id ? 'border-[#1A1A2E] ring-2 ring-[#1A1A2E]/20 scale-110' : 'border-slate-200'}`}
                        ...
                        {product.available_sizes?.length > 0 && (
                            <div className="mb-8">
                                <div className="mb-3 flex items-center justify-between">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-[#1A1A2E]">Size</span>
                                    <button className="text-[10px] font-bold text-slate-400 underline uppercase tracking-widest hover:text-[#1A1A2E]">Size Guide</button>
                                </div>
                                <div className="grid grid-cols-4 gap-3">
                                    {product.available_sizes.map(size => (
                                        <button
                                            ...
                                            className={`border py-3 text-[10px] font-black uppercase tracking-widest transition-colors ${selectedSize?.id === size.id ? 'border-[#1A1A2E] bg-[#1A1A2E] text-white' : 'border-slate-200 bg-white text-[#1A1A2E] hover:border-[#1A1A2E]'}`}
                                        >
                                            {size.code}
                                        </button>
                                    ))}
                        ...
                        <div className="mb-8 flex flex-col gap-4 sm:flex-row">
                            ...
                            <button
                                ...
                                disabled={!inStock}
                                className="flex h-14 flex-1 items-center justify-center gap-2 bg-[#1A1A2E] text-xs font-black uppercase tracking-[0.2em] text-white transition-colors hover:bg-[#E94E3C] disabled:bg-slate-200 disabled:text-slate-400"
                            >
                                <ShoppingBag size={16} /> {inStock ? 'Add to Cart' : 'Out of Stock'}
                            </button>
                            <button className="flex h-14 w-14 items-center justify-center border border-slate-200 bg-white text-[#1A1A2E] transition-colors hover:border-[#1A1A2E]">
                                <Heart size={20} />
                            </button>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-4 border-t border-slate-100 pt-8">
                            <div className="flex flex-col items-center gap-2 text-center">
                                <Truck size={24} strokeWidth={1.5} className="text-[#1A1A2E]" />
                            ...
                            <div className="flex flex-col items-center gap-2 text-center border-x border-slate-100">
                                <RotateCcw size={24} strokeWidth={1.5} className="text-[#1A1A2E]" />
                                <span className="text-[9px] font-bold uppercase tracking-widest text-slate-500">7-Day Returns</span>
                            </div>
                            <div className="flex flex-col items-center gap-2 text-center">
                                <ShieldCheck size={24} strokeWidth={1.5} className="text-[#1A1A2E]" />
                ...
                {relatedProducts?.length > 0 && (
                    <div className="mt-24 border-t border-slate-100 pt-16">
                        <div className="mb-10 flex items-end justify-between">
                            <h2 className="text-2xl font-black uppercase tracking-tight text-[#1A1A2E]">Complete The Kit</h2>
                        </div>
