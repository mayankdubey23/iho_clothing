...
export default function StockRequests({ requests, stats, filters }) {
    ...
    const statusConfig = {
        ...
        completed: { label: 'Completed', color: 'bg-[#282c3f] text-white border-[#282c3f]' },
            ...
            <div className="max-w-[1600px] mx-auto pb-20 pt-6 px-4 sm:px-6">
                ...
                <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-black text-[#282c3f] uppercase tracking-tighter flex items-center gap-3">
                            <PackageSearch className="text-[#ff3f6c]" size={28} /> Stock Requests
                        </h1>
                ...
                <div className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row items-center gap-4">
                    <div className="flex-1 w-full relative">
                        ...
                        <input type="text" placeholder="Search Request ID or Franchise Name..." value={search} onChange={e => setSearch(e.target.value)} onKeyPress={handleKeyPress} className="w-full bg-gray-50 border-none rounded-xl pl-12 pr-4 py-3 text-sm font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none" />
                    </div>
                                ...
                                {requests?.data?.length > 0 ? requests.data.map((req) => (
                                    <tr key={req.id} className="hover:bg-gray-50/80 transition-colors group">
                                        <td className="px-6 py-4">
                                            <p className="font-black text-[#282c3f] text-sm">REQ-{req.id.toString().padStart(4, '0')}</p>
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="font-bold text-[#282c3f] text-sm flex items-center gap-1.5"><Store size={14} className="text-[#ff3f6c]" /> {req.franchise_name}</p>
                                        </td>

                                        <td className="px-6 py-4 text-center">
                                            <span className="bg-gray-100 text-[#282c3f] font-black text-xs px-3 py-1 rounded-lg border border-gray-200">{req.items_count || 0} Units</span>
                                        </td>

                                        <td className="px-6 py-4">
                                            <p className="font-black text-[#282c3f]">₹{Number(req.total_amount).toLocaleString()}</p>
                                                ...
                                                {openActionMenu === req.id && (
                                                    <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="absolute right-12 top-2 w-48 bg-white border border-gray-100 shadow-[0_10px_40px_rgba(0,0,0,0.1)] rounded-2xl z-50 overflow-hidden text-left">
                                                        <div className="p-2 space-y-1">
                                                            <button onClick={() => { setDetailsRequest(req); setOpenActionMenu(null); }} className="block w-full text-left px-4 py-2.5 text-xs font-black uppercase tracking-widest text-[#282c3f] hover:bg-gray-50 rounded-xl transition-colors">View Items</button>
                                    ...
                                    <tr><td colSpan="6" className="px-6 py-16 text-center"><PackageSearch size={48} className="mx-auto text-gray-300 mb-4" strokeWidth={1} /><p className="text-[#282c3f] font-black text-lg">No Stock Requests</p></td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#282c3f]/60 p-4 backdrop-blur-sm">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
                            <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50 p-6">
                                <div>
                                    <h3 className="font-black uppercase tracking-wider text-[#282c3f]">{detailsRequest.request_number || `REQ-${detailsRequest.id}`}</h3>
                                ...
                                <div className="space-y-3">
                                    {detailsRequest.items?.map((item) => (
                                        <div key={item.id} className="grid grid-cols-[1fr_auto] gap-4 rounded-2xl border border-gray-100 bg-gray-50 p-4">
                                            <div>
                                                <p className="font-black text-[#282c3f]">{item.product_name}</p>
                                                <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-gray-400">Qty {item.quantity}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-sm font-black text-[#282c3f]">Rs {Number(item.total_price || 0).toLocaleString()}</p>
