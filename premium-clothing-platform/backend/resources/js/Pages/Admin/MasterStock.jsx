    ...
    return (
        <AdminLayout active="inventory">
            ...
            <div className="max-w-[1600px] mx-auto pb-20 pt-6 px-4 sm:px-6">
                ...
                {/* 🚀 HEADER & STATS */}
                <div className="mb-8">
                    <h1 className="text-3xl font-black text-[#1A1A2E] uppercase tracking-tighter mb-6 flex items-center gap-3">
                        <Box className="text-[#E94E3C]" /> Master Stock
                    </h1>
                                ...
                                {safeInventory.length > 0 ? safeInventory.map((item) => (
                                    <tr key={item.id} className="hover:bg-gray-50/80 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className="font-black text-[#1A1A2E] text-sm max-w-[250px] truncate">{item.name}</p>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`text-lg font-black ${item.stock_quantity <= 10 ? 'text-red-500' : 'text-[#1A1A2E]'}`}>{item.stock_quantity || 0}</span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                <button onClick={() => { setTransferModal({ open: true, product: item }); }} className="p-2 text-[#E94E3C] bg-[#E94E3C]/10 hover:bg-[#E94E3C]/20 rounded-xl transition-colors tooltip" title="Transfer to Franchise"><ArrowRightLeft size={16} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                )) : (
                                    <tr>
                                        <td colSpan="6" className="px-6 py-16 text-center">
                                            ...
                                            <p className="text-[#1A1A2E] font-black text-lg">Inventory Empty</p>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#1A1A2E]/60 backdrop-blur-sm">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                                <h3 className="font-black text-[#1A1A2E] uppercase tracking-wider">Adjust Stock</h3>
                            ...
                            <form onSubmit={handleAdjustSubmit} className="p-6 space-y-5">
                                ...
                                <div>
                                    ...
                                    <input type="number" min="1" required value={adjustData.quantity} onChange={e => setAdjustData('quantity', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#1A1A2E] focus:ring-2 focus:ring-[#E94E3C] outline-none" placeholder="Enter quantity..." />
                                ...
                                <div>
                                    ...
                                    <input type="text" value={adjustData.reason} onChange={e => setAdjustData('reason', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#1A1A2E] focus:ring-2 focus:ring-[#E94E3C] outline-none" placeholder="e.g. Supplier delivery, Audit adjustment..." />
                                </div>

                                <button disabled={adjustProcessing} type="submit" className="w-full bg-[#1A1A2E] text-white py-3.5 rounded-xl font-black uppercase tracking-widest hover:bg-[#E94E3C] transition-colors disabled:opacity-50 mt-4">
                            ...
                            <form onSubmit={handleTransferSubmit} className="p-6 space-y-5">
                                <div className="bg-gray-50 p-4 rounded-2xl flex justify-between items-center mb-4">
                                    <div>
                                        ...
                                        <p className="text-xl font-black text-[#1A1A2E]">{transferModal.product?.stock_quantity || 0} Units</p>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <select required value={transferData.franchise_id} onChange={e => setTransferData('franchise_id', e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#1A1A2E] focus:ring-2 focus:ring-[#E94E3C] outline-none">
                                ...
                                <div>
                                    ...
                                    <input type="number" min="1" max={transferModal.product?.stock_quantity} required value={transferData.quantity} onChange={e => setTransferData('quantity', e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#1A1A2E] focus:ring-2 focus:ring-[#E94E3C] outline-none" placeholder="Units to transfer..." />
                                    {transferErrors.quantity && <p className="text-[10px] text-red-500 mt-1 font-bold">{transferErrors.quantity}</p>}
                                </div>

                                <button disabled={transferProcessing} type="submit" className="w-full bg-[#E94E3C] text-white py-3.5 rounded-xl font-black uppercase tracking-widest hover:bg-[#c0392b] transition-colors disabled:opacity-50 mt-4 flex items-center justify-center gap-2">
...
function TypeBtn({ active, onClick, color, icon, children }) {
    return (
        <button type="button" onClick={onClick} className={`flex-1 flex items-center justify-center gap-1 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${active ? color + ' ring-2 ring-offset-1 ring-[#1A1A2E]' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}>
            {icon} {children}
        </button>
    );
}
