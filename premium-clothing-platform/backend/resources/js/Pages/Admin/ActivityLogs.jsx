...
export default function ActivityLogs({ logs, modules, stats, filters }) {
    ...
    return (
        <AdminLayout active="logs">
            ...
            <div className="max-w-[1600px] mx-auto pb-20 pt-6 px-4 sm:px-6">
                ...
                <div className="mb-8">
                    <h1 className="text-3xl font-black text-[#282c3f] uppercase tracking-tighter flex items-center gap-3">
                        <ShieldAlert className="text-[#ff3f6c]" size={32} /> Master Audit Trail
                    </h1>
                ...
                <div className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row items-center gap-4">
                    <div className="flex-1 w-full relative">
                        ...
                        <input type="text" placeholder="Search Staff Name, Action, or IP Address..." value={search} onChange={e => setSearch(e.target.value)} onKeyPress={handleKeyPress} className="w-full bg-gray-50 border-none rounded-xl pl-12 pr-4 py-3 text-sm font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none" />
                    </div>
                    <select value={moduleFilter} onChange={e => { setModuleFilter(e.target.value); setTimeout(applyFilters, 100); }} className="bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-gray-600 focus:ring-2 focus:ring-[#ff3f6c] outline-none cursor-pointer">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-visible min-h-[400px]">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-[#282c3f] text-[10px] uppercase font-black tracking-[0.2em] text-white/70 border-b border-[#282c3f]">
                                <tr>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {logs?.data?.map((log) => (
                                    <tr key={log.id} className="hover:bg-gray-50/80 transition-colors">
                                        <td className="px-6 py-4">
                                            <p className="font-black text-[#282c3f] text-xs flex items-center gap-1.5"><Clock size={12} className="text-[#ff3f6c]" /> {new Date(log.created_at).toLocaleString()}</p>
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="font-bold text-[#282c3f] text-sm">{log.user_name}</p>
                                ...
                                {logs?.data?.length === 0 && (
                                    <tr><td colSpan="4" className="px-6 py-16 text-center"><ShieldAlert size={48} className="mx-auto text-gray-300 mb-4" strokeWidth={1} /><p className="text-[#282c3f] font-black text-lg">No Audit Logs Found</p></td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
            ...
            <AnimatePresence>
                {selectedLog && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#282c3f]/80 backdrop-blur-sm">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden max-h-[90vh] flex flex-col">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-[#282c3f]">
                                <div>
                                    <h3 className="font-black text-white uppercase tracking-wider flex items-center gap-2"><Database size={18} className="text-[#ff3f6c]" /> Data Inspector</h3>
                                ...
                                {selectedLog.device_info && (
                                    <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs font-bold text-[#282c3f] mt-1 break-all">{selectedLog.device_info}</p>
                                        </div>
                                    </div>
                                )}
    ...
    return (
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div>
                ...
                <h4 className="text-3xl font-black text-[#282c3f]">{value?.toLocaleString() || 0}</h4>
            </div>
