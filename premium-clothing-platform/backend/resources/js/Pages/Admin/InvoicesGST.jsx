        ...
        <AdminLayout active="billing">
            ...
            <div className="max-w-[1600px] mx-auto pb-20 pt-6 px-4 sm:px-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-[#282c3f] uppercase tracking-tighter flex items-center gap-3">
                            <Receipt className="text-[#ff3f6c]" size={32} /> Billing & GST
                        </h1>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-2 mb-6 flex gap-2 w-max">
                    <button onClick={() => switchTab('invoices')} className={`flex items-center gap-2 px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'invoices' ? 'bg-[#282c3f] text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>
                        <FileText size={16} /> Invoice Ledger
                    </button>
                    <button onClick={() => switchTab('settings')} className={`flex items-center gap-2 px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'settings' ? 'bg-[#282c3f] text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-visible min-h-[400px]">
                    ...
                    {activeTab === 'invoices' && (
                        <>
                            <div className="p-4 border-b border-gray-100 relative">
                                ...
                                <input type="text" placeholder="Search Invoice Number or Billing Name..." value={search} onChange={e => setSearch(e.target.value)} onKeyPress={handleSearch} className="w-full bg-gray-50 border-none rounded-xl pl-12 pr-4 py-3 text-sm font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none" />
                            </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {invoices?.data?.map((inv) => (
                                            <tr key={inv.id} className="hover:bg-gray-50/80 transition-colors">
                                                <td className="px-6 py-4">
                                                    <p className="font-black text-[#282c3f] text-sm flex items-center gap-1.5"><Receipt size={14} className="text-gray-400" /> {inv.invoice_number}</p>
                                                ...
                                                <td className="px-6 py-4">
                                                    <p className="font-black text-[#282c3f] text-sm">{inv.billing_name}</p>
                                                ...
                                                <td className="px-6 py-4">
                                                    <p className="font-black text-[#282c3f] text-sm">₹{Number(inv.final_amount).toLocaleString()}</p>
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex justify-end gap-2">
                                                        ...
                                                        <button onClick={() => downloadInvoice(inv.id)} className="p-2 text-[#282c3f] bg-gray-100 hover:bg-[#282c3f] hover:text-white rounded-xl transition-all" title="Download PDF">
                                                            <Download size={16} />
                                                        </button>
                                                    </div>
                                                </td>
                                        ...
                                        {invoices?.data?.length === 0 && (
                                            <tr><td colSpan="5" className="px-6 py-16 text-center"><Receipt size={48} className="mx-auto text-gray-300 mb-4" strokeWidth={1} /><p className="text-[#282c3f] font-black text-lg">No Invoices Found</p></td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                    ...
                    {activeTab === 'settings' && (
                        <form onSubmit={saveSettings} className="p-8 max-w-3xl mx-auto space-y-6">
                            ...
                            <div className="bg-[#282c3f] text-white p-6 rounded-2xl flex items-center gap-4 shadow-xl shadow-black/10 mb-8">
                                <Landmark size={32} className="text-[#ff3f6c]" />
                                <div>
                            ...
                            <div className="space-y-1.5 border-t border-gray-100 pt-6">
                                ...
                                <div className="relative w-1/3">
                                    ...
                                    <input type="number" step="0.01" required value={data.default_tax_percentage} onChange={e => setData('default_tax_percentage', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-12 pr-4 py-3 font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none" />
                                </div>
                            ...
                            <div className="space-y-1.5 border-t border-gray-100 pt-6">
                                ...
                                <textarea rows="3" value={data.billing_address} onChange={e => setData('billing_address', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none resize-none"></textarea>
                            </div>

                            <div className="pt-4 text-right">
                                <button disabled={processing} type="submit" className="bg-[#ff3f6c] text-white px-8 py-3.5 rounded-xl font-black uppercase tracking-[0.1em] hover:bg-[#c0392b] transition-all disabled:opacity-50">
