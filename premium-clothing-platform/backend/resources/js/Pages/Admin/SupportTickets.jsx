            ...
            <div className="max-w-[1600px] mx-auto pb-20 pt-6 px-4 sm:px-6">
                ...
                <div className="mb-8">
                    <h1 className="text-3xl font-black text-[#282c3f] uppercase tracking-tighter flex items-center gap-3">
                        <LifeBuoy className="text-[#ff3f6c]" size={32} /> Global Helpdesk
                    </h1>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-2 mb-6 flex gap-2">
                    <button onClick={() => applyFilters('Customer')} className={`flex-1 flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'Customer' ? 'bg-[#282c3f] text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>
                        <Users size={16} /> Customer Tickets
                    </button>
                    <button onClick={() => applyFilters('Franchise')} className={`flex-1 flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'Franchise' ? 'bg-[#282c3f] text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>
                ...
                <div className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row items-center gap-4">
                    <div className="flex-1 w-full relative">
                        ...
                        <input type="text" placeholder="Search Ticket #, Subject or Name..." value={search} onChange={e => setSearch(e.target.value)} onKeyPress={e => e.key === 'Enter' && applyFilters()} className="w-full bg-gray-50 border-none rounded-xl pl-12 pr-4 py-3 text-sm font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none" />
                    </div>
                    <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); applyFilters(activeTab, e.target.value); }} className="bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-gray-600 focus:ring-2 focus:ring-[#ff3f6c] outline-none cursor-pointer">
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {tickets?.data?.map((ticket) => (
                                    <tr key={ticket.id} className={`hover:bg-gray-50/80 transition-colors group cursor-pointer ${ticket.status === 'Open' ? 'bg-orange-50/30' : ''}`} onClick={() => router.get(`/franchise-superadmin/tickets/${ticket.id}`)}>
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="font-black text-[#282c3f] text-sm group-hover:text-[#ff3f6c] transition-colors">{ticket.subject}</p>
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="font-bold text-[#282c3f] text-sm">{ticket.creator_name}</p>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button className="px-4 py-2 bg-gray-50 text-gray-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#282c3f] hover:text-white transition-colors group-hover:shadow-md">
                                                View Thread
                                            </button>
                                        </td>
                                    </tr>
                                ...
                                {tickets?.data?.length === 0 && (
                                    <tr><td colSpan="5" className="px-6 py-16 text-center"><MessageSquare size={48} className="mx-auto text-gray-300 mb-4" strokeWidth={1} /><p className="text-[#282c3f] font-black text-lg">Inbox Zero! 🎉</p></td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
