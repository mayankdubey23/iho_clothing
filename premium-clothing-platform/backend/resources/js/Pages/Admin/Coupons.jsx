...
export default function Coupons({ coupons, stats, filters, storeOffers }) {
    ...
    const createOffer = (e) => {
        e.preventDefault();
        postOffer('/franchise-superadmin/offers', { preserveScroll: true, onSuccess: () => resetOffer() });
    };
        ...
        <AdminLayout active="promotions"> {/* Update sidebar active state if needed */}
            ...
            <div className="max-w-[1600px] mx-auto pb-20 pt-6 px-4 sm:px-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-[#282c3f] uppercase tracking-tighter flex items-center gap-3">
                            <Tag className="text-[#ff3f6c]" /> Promotions & Offers
                        </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    ...
                    <StatCard title="Most Popular Code" value={stats?.most_used_code} icon={Percent} color="text-[#ff3f6c]" />
                </div>
                ...
                <div className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 mb-6 flex items-center gap-4">
                    <div className="flex-1 relative">
                        ...
                        <input type="text" placeholder="Search Promo Codes (e.g. SPORTS10)..." value={search} onChange={e => setSearch(e.target.value)} onKeyPress={e => e.key === 'Enter' && router.get('/franchise-superadmin/coupons', { search })} className="w-full bg-gray-50 border-none rounded-xl pl-12 pr-4 py-3 text-sm font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none uppercase" />
                    </div>
                </div>
                                ...
                                {coupons?.data?.length > 0 ? coupons.data.map((coupon) => (
                                    <tr key={coupon.id} className={`hover:bg-gray-50/80 transition-colors group ${coupon.status === 'inactive' ? 'opacity-60' : ''}`}>
                                        <td className="px-6 py-4">
                                            <div className="inline-flex items-center gap-2 border-2 border-dashed border-[#282c3f] bg-gray-50 px-3 py-1.5 rounded-lg">
                                                <Tag size={12} className="text-[#ff3f6c]" />
                                                <span className="font-black text-[#282c3f] text-sm uppercase tracking-widest">{coupon.code}</span>
                                            </div>
                                            {coupon.expiry_date && (
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="font-black text-[#282c3f] text-base flex items-center gap-1">
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className="bg-gray-100 text-[#282c3f] font-black text-xs px-3 py-1.5 rounded-lg border border-gray-200">
                                    ...
                                    <tr><td colSpan="5" className="px-6 py-16 text-center"><Tag size={48} className="mx-auto text-gray-300 mb-4" strokeWidth={1} /><p className="text-[#282c3f] font-black text-lg">No Coupons Found</p></td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                ...
                <div className="mb-6 border-b border-gray-200 pb-4">
                    <h2 className="text-2xl font-black text-[#282c3f] uppercase tracking-tighter flex items-center gap-2">
                        <LayoutTemplate size={24} className="text-[#ff3f6c]" />
                ...
                <form onSubmit={createOffer} className="mb-6 rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
                    <div className="mb-5 flex items-center justify-between gap-4">
                        <h3 className="text-sm font-black uppercase tracking-[0.22em] text-[#282c3f]">Add Offer Banner</h3>
                        <button type="submit" disabled={offerProcessing} className="inline-flex items-center gap-2 rounded-xl bg-[#282c3f] px-5 py-3 text-xs font-black uppercase tracking-widest text-white transition-colors hover:bg-[#ff3f6c] disabled:opacity-50">
                            <Plus size={16} /> Add Banner
                        </button>
                    </div>
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-4">
                        <InputField label="Title" value={offerData.title} onChange={e => setOfferData('title', e.target.value)} error={offerErrors.title} required />
                        <InputField label="Subtitle" value={offerData.subtitle} onChange={e => setOfferData('subtitle', e.target.value)} error={offerErrors.subtitle} />
                        <InputField label="Coupon Code" value={offerData.offer_code} onChange={e => setOfferData('offer_code', e.target.value.toUpperCase())} error={offerErrors.offer_code} />
                        <InputField label="Image URL / Storage Path" value={offerData.bg_image} onChange={e => setOfferData('bg_image', e.target.value)} error={offerErrors.bg_image} placeholder="offers/banner.jpg" />
                        <InputField label="Target Link" value={offerData.target_url} onChange={e => setOfferData('target_url', e.target.value)} error={offerErrors.target_url} placeholder="/shop" />
                        <InputField label="Display Type" value={offerData.display_type} onChange={e => setOfferData('display_type', e.target.value)} error={offerErrors.display_type} placeholder="hero_banner" />
                        <InputField label="Sort Order" type="number" min="0" value={offerData.sort_order} onChange={e => setOfferData('sort_order', e.target.value)} error={offerErrors.sort_order} />
                        <label className="flex items-center gap-2 pt-7 text-xs font-black uppercase tracking-widest text-[#282c3f]">
                            <input type="checkbox" checked={offerData.is_active} onChange={e => setOfferData('is_active', e.target.checked)} className="size-5 rounded-md border-gray-300 text-[#ff3f6c] focus:ring-[#ff3f6c]" />
                            Show on Website
                        </label>
                    </div>
                </form>
                ...
                <div className="grid gap-6 pb-12">
                    {storefrontOffers.length > 0 ? (
                        ...
                        <div className="rounded-3xl border border-dashed border-gray-200 bg-white p-10 text-center text-xs font-black uppercase tracking-widest text-gray-400">
                            No offer banners yet. Add one above to show it on the homepage.
                        </div>
                    )}
                </div>
                {/* ========================================================= */}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#282c3f]/60 backdrop-blur-sm">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] overflow-y-auto custom-scrollbar">
                            <div className="sticky top-0 bg-white/90 backdrop-blur-md p-6 border-b border-gray-100 flex justify-between items-center z-10">
                                <h3 className="font-black text-[#282c3f] uppercase tracking-wider flex items-center gap-2"><Tag size={18} className="text-[#ff3f6c]" /> Create Discount Coupon</h3>
                            ...
                            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <select value={data.type} onChange={e => setData('type', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none cursor-pointer">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="space-y-1.5">
                                        ...
                                        <select value={data.target_audience} onChange={e => setData('target_audience', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none cursor-pointer">
                                ...
                                <button disabled={processing} type="submit" className="w-full bg-[#282c3f] text-white py-4 rounded-xl font-black uppercase tracking-widest hover:bg-[#ff3f6c] transition-colors disabled:opacity-50 shadow-xl shadow-black/10 mt-2">
...
function InputField({ label, error, ...props }) {
    return (
        <div className="space-y-1.5">
            ...
            <input className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none transition-all disabled:opacity-50 disabled:bg-gray-200" {...props} />
...
function StoreOfferEditCard({ offer }) {
    ...
    const { data, setData, put, processing } = useForm({
        ...
        display_type: offer.display_type || 'store_offer',
        bg_image: offer.bg_image || '',
        target_url: offer.target_url || '/shop',
    ...
    const deleteOffer = () => {
        if (confirm('Delete this storefront offer banner?')) {
            router.delete(`/franchise-superadmin/offers/${offer.id}`, { preserveScroll: true });
        }
    };

    return (
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between transition-colors hover:border-[#282c3f]/20">
            {!isEditing ? (
                <>
                    <div className="flex-1">
                        <span className="text-[10px] font-black uppercase tracking-widest text-[#ff3f6c] bg-[#ff3f6c]/10 px-2 py-1 rounded-md mb-3 inline-block">
                            {(offer.display_type || 'store_offer').replace('_', ' ')}
                        </span>
                        <h3 className="text-xl font-black text-[#282c3f] uppercase">{offer.title}</h3>
                        ...
                        {(offer.offer_code || offer.code) && (
                            <p className="mt-3 text-xs font-black text-gray-400 uppercase tracking-widest">
                                Linked Code: <span className="bg-gray-50 px-2 py-1 rounded text-[#282c3f]">{offer.offer_code || offer.code}</span>
                            </p>
                        )}
                        {offer.bg_image && (
                            <p className="mt-2 text-xs font-black text-gray-400 uppercase tracking-widest">
                                Image: <span className="normal-case tracking-normal text-[#282c3f]">{offer.bg_image}</span>
                            </p>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-4 w-full md:w-auto">
                        ...
                        <button onClick={() => setIsEditing(true)} className="w-full md:w-auto bg-gray-50 text-[#282c3f] px-6 py-3 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-[#282c3f] hover:text-white transition-all border border-gray-200">
                            Edit Content
                        </button>
                        <button onClick={deleteOffer} className="w-full md:w-auto bg-red-50 text-red-500 px-4 py-3 text-xs font-black uppercase tracking-widest rounded-xl hover:bg-red-500 hover:text-white transition-all border border-red-100">
                            <Trash2 size={16} />
                        </button>
                    </div>
                </>
                ...
                <form onSubmit={submit} className="w-full flex flex-col gap-5">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                        ...
                        <InputField label="Display Type" value={data.display_type} onChange={e => setData('display_type', e.target.value)} placeholder="hero_banner" />
                        <InputField label="Image URL / Storage Path" value={data.bg_image} onChange={e => setData('bg_image', e.target.value)} placeholder="offers/banner.jpg" />
                        <InputField label="Target Link" value={data.target_url} onChange={e => setData('target_url', e.target.value)} placeholder="/shop" />
                        <InputField label="Sort Order" type="number" min="0" value={data.sort_order} onChange={e => setData('sort_order', e.target.value)} />
                    </div>

                    <div className="flex flex-col sm:flex-row items-center justify-between mt-2 pt-5 border-t border-gray-100 gap-4">
                        <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={data.is_active} onChange={e => setData('is_active', e.target.checked)} className="text-[#ff3f6c] rounded-md focus:ring-[#ff3f6c] size-5 border-gray-300 transition-colors" />
                            <span className="text-xs font-black uppercase tracking-widest text-[#282c3f]">Show on Website</span>
                        </label>
                        ...
                        <div className="flex w-full sm:w-auto gap-3">
                            ...
                            <button type="submit" disabled={processing} className="flex-1 sm:flex-none flex items-center justify-center gap-2 text-xs font-black uppercase tracking-widest bg-[#282c3f] text-white px-8 py-3 rounded-xl hover:bg-[#ff3f6c] transition-colors disabled:opacity-50">
