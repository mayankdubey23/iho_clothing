            ...
            <div className="max-w-[1600px] mx-auto pb-20 pt-6 px-4 sm:px-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-[#282c3f] uppercase tracking-tighter flex items-center gap-3">
                            <Store className="text-[#ff3f6c]" size={32} /> Store Catalog
                        </h1>
                        ...
                        <div key={product.mapping_id} className={`bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden transition-all hover:shadow-lg ${!product.is_active_locally ? 'opacity-70 grayscale-[30%]' : ''}`}>
                            ...
                            <div className="h-48 bg-gray-50 relative border-b border-gray-100 overflow-hidden group">
                                ...
                                <div className="absolute top-3 left-3 flex flex-col gap-2">
                                    <span className="bg-white/90 backdrop-blur text-[#282c3f] text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md shadow-sm border border-gray-200/50">
                            ...
                            <div className="p-5">
                                ...
                                <h3 className="font-black text-[#282c3f] text-lg leading-tight mb-4 truncate">{product.name}</h3>
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-5">
                                    ...
                                    <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                                        ...
                                        <p className="font-black text-[#282c3f] mt-0.5">{product.available_stock} Units</p>
                                    </div>
                                </div>

                                <div className="flex items-center justify-between pt-4 border-t border-dashed border-gray-200">
                                    <div>
                                        <p className="text-[9px] font-black text-[#ff3f6c] uppercase tracking-widest">Local Selling Price</p>
                                        <p className="font-black text-[#282c3f] text-xl flex items-center gap-1 mt-0.5">
                                            <IndianRupee size={16} strokeWidth={3} />
                                            {Number(product.local_selling_price || product.mrp).toLocaleString()}
                                        </p>
                                    </div>
                                    <button onClick={() => openEditModal(product)} className="text-[10px] bg-[#282c3f] text-white hover:bg-[#ff3f6c] font-black uppercase tracking-widest px-4 py-2 rounded-lg transition-all shadow-md">
                                        Edit Price
                                    </button>
                                </div>
                            </div>
                ...
                {products?.data?.length === 0 && (
                    <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm mt-6">
                        ...
                        <h3 className="text-[#282c3f] font-black text-xl uppercase tracking-wider mb-2">No Products Assigned</h3>
                        ...
                        >
                            ...
                            <form onSubmit={submitPriceUpdate} className="p-6 space-y-5">
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 text-center mb-2">
                                    ...
                                    <p className="font-black text-[#282c3f] text-sm leading-tight">{editingProduct.name}</p>
                                    <div className="mt-3 inline-block bg-white border border-gray-200 px-3 py-1 rounded-md text-[10px] font-bold text-gray-500">
                                        Master MRP: <span className="font-black text-[#282c3f]">₹{editingProduct.mrp}</span>
                                    </div>
                                </div>

                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black text-[#ff3f6c] uppercase tracking-widest ml-1">Your Local Selling Price *</label>
                                    <div className="relative">
                                        <IndianRupee size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                                        <input required type="number" step="0.01" value={data.local_selling_price} onChange={e => setData('local_selling_price', e.target.value)} className="w-full bg-white border border-gray-200 rounded-xl pl-12 pr-4 py-3 font-black text-xl text-[#282c3f] focus:ring-2 focus:ring-[#ff3f6c] outline-none" />
                                    </div>
                                    <p className="text-[9px] font-bold text-gray-400 ml-1 mt-1">This price will be shown to customers in your service area.</p>
                                </div>

                                <button disabled={processing} type="submit" className="w-full bg-[#ff3f6c] text-white py-3.5 rounded-xl font-black uppercase tracking-widest hover:bg-[#c0392b] transition-colors mt-2 shadow-xl shadow-[#ff3f6c]/20 flex items-center justify-center gap-2">
