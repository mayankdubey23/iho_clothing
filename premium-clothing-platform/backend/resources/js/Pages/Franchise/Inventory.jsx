            ...
            <div className="max-w-[1600px] mx-auto pb-20 pt-6 px-4 sm:px-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-[#1A1A2E] uppercase tracking-tighter flex items-center gap-3">
                            <PackageSearch className="text-[#E94E3C]" size={32} /> My Inventory
                        </h1>
                        <p className="text-gray-500 font-bold text-sm mt-1">Manage variants, track performance, and report damages.</p>
                    </div>
                    <div className="flex gap-3">
                        <button className="bg-white border border-gray-200 text-[#1A1A2E] px-4 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-gray-50 transition-all flex items-center gap-2 shadow-sm">
                            <FileDown size={18} /> Report
                        </button>
                        <Link href="/franchise/stock-requests/new" className="bg-[#1A1A2E] text-white px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-[#E94E3C] transition-all flex items-center gap-2 shadow-lg">
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-6 justify-between items-center bg-white p-3 rounded-3xl shadow-sm border border-gray-100">
                    <div className="flex gap-2 w-full md:w-auto">
                        <button onClick={() => applyFilter('all')} className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${filters.filter === 'all' ? 'bg-[#1A1A2E] text-white' : 'bg-gray-50 text-gray-500'}`}>All Items</button>
                    ...
                    <div className="relative w-full md:w-80">
                        ...
                        <input type="text" placeholder="Search Product or SKU..." value={search} onChange={e => setSearch(e.target.value)} onKeyPress={handleSearch} className="w-full bg-gray-50 border border-gray-100 rounded-xl pl-12 pr-4 py-3 font-bold text-[#1A1A2E] focus:ring-2 focus:ring-[#E94E3C] outline-none" />
                    </div>
                </div>
                                    ...
                                    return (
                                        <tr key={item.id} className="hover:bg-gray-50/80 transition-colors">
                                            <td className="px-6 py-4">
                                                <p className="font-bold text-[#1A1A2E] text-sm">{item.product_name}</p>
                                                <p className="text-[10px] font-black text-gray-400 uppercase mt-0.5">SKU: {item.variant_sku || item.master_sku}</p>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex gap-2">
                                                    {item.size ? <span className="bg-gray-100 text-[#1A1A2E] px-2 py-1 rounded text-[10px] font-black uppercase">{item.size}</span> : <span className="text-gray-400 text-xs">-</span>}
                                                    {item.color && <span className="bg-gray-100 text-[#1A1A2E] px-2 py-1 rounded text-[10px] font-black uppercase">{item.color}</span>}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-center">
                                                <span className={`text-xl font-black ${isOut ? 'text-red-600' : (isLow ? 'text-orange-500' : 'text-[#1A1A2E]')}`}>{item.quantity}</span>
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
                            ...
                            <form onSubmit={submitDamaged} className="p-6 space-y-5">
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 text-center mb-2">
                                    <p className="font-black text-[#1A1A2E] text-sm">{damagedModal.product_name}</p>
    ...
    return (
        <div className={`bg-white p-4 md:p-6 rounded-3xl border ${alert ? 'border-red-200 shadow-red-500/10' : 'border-gray-100'} shadow-sm flex items-center justify-between`}>
            <div>
                ...
                <h4 className="text-2xl md:text-3xl font-black text-[#1A1A2E]">{value || 0}</h4>
            </div>
