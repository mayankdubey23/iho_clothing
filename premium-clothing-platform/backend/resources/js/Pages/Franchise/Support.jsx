                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-[#1A1A2E] uppercase tracking-tighter flex items-center gap-3">
                            <LifeBuoy className="text-[#E94E3C]" size={32} /> Helpdesk
                        </h1>
                        <p className="text-gray-500 font-bold text-sm mt-1">Raise issues and communicate directly with the Super Admin.</p>
                    </div>
                    <button
                        onClick={() => setIsCreateModalOpen(true)}
                        className="bg-[#1A1A2E] text-white px-6 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-[#E94E3C] transition-all flex items-center gap-2 shadow-xl shadow-[#1A1A2E]/20"
                    >
                    ...
                    <div className="lg:col-span-1 space-y-4 h-[75vh] overflow-y-auto pr-2 custom-scrollbar">
                        {tickets.length > 0 ? tickets.map((ticket) => (
                            <div
                                ...
                                className={`bg-white p-5 rounded-2xl border-2 transition-all cursor-pointer ${activeTicket?.id === ticket.id ? 'border-[#E94E3C] shadow-md' : 'border-transparent shadow-sm hover:border-gray-200'}`}
                            >
                                ...
                                <h4 className="font-bold text-[#1A1A2E] text-sm line-clamp-1 mb-1">{ticket.subject}</h4>
                            ...
                            <div className="bg-white rounded-3xl p-10 text-center shadow-sm border border-gray-100">
                                ...
                                <p className="font-black text-[#1A1A2E] uppercase">All Good!</p>
                        ...
                        {activeTicket ? (
                            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 h-[75vh] flex flex-col overflow-hidden relative">
                                ...
                                <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center shrink-0">
                                    <div>
                                        <h2 className="font-black text-[#1A1A2E] text-lg">{activeTicket.subject}</h2>
                                ...
                                <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#f9f8f6] custom-scrollbar">
                                    {activeTicket.messages.map((msg) => (
                                        <div key={msg.id} className={`flex ${msg.is_admin_reply ? 'justify-start' : 'justify-end'}`}>
                                            <div className={`max-w-[80%] md:max-w-[70%] rounded-2xl p-4 shadow-sm ${msg.is_admin_reply ? 'bg-white border border-gray-100 rounded-tl-sm' : 'bg-[#1A1A2E] text-white rounded-tr-sm'}`}>
                                                <div className="flex justify-between items-center mb-2 gap-4">
                                                    <span className={`text-[10px] font-black uppercase tracking-widest ${msg.is_admin_reply ? 'text-indigo-600' : 'text-[#E94E3C]'}`}>
                                                ...
                                                {msg.attachment_path && (
                                                    <a href={`/storage/${msg.attachment_path}`} target="_blank" className={`mt-3 inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg ${msg.is_admin_reply ? 'bg-gray-50 text-[#1A1A2E]' : 'bg-white/10 text-white hover:bg-white/20'}`}>
                                ...
                                {activeTicket.status !== 'Closed' ? (
                                    <form onSubmit={submitReply} className="p-4 bg-white border-t border-gray-100 shrink-0">
                                        <div className="flex items-end gap-3">
                                            <div className="flex-1 bg-gray-50 rounded-2xl border border-gray-200 p-2 focus-within:border-[#E94E3C] focus-within:ring-1 focus-within:ring-[#E94E3C] transition-all">
                                                <textarea required rows="2" placeholder="Type your reply here..." value={replyData.message} onChange={e => setReplyData('message', e.target.value)} className="w-full bg-transparent border-none focus:ring-0 resize-none text-sm font-medium text-[#1A1A2E] p-2" />
                                                <div className="flex justify-between items-center px-2 pb-1">
                                                    <label className="cursor-pointer text-gray-400 hover:text-[#1A1A2E] transition-colors flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest">
                                            ...
                                            <button disabled={replying} type="submit" className="bg-[#E94E3C] text-white size-14 rounded-2xl flex items-center justify-center hover:bg-[#c0392b] transition-colors shadow-lg shadow-[#E94E3C]/20 shrink-0">
                            ...
                            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 h-[75vh] flex flex-col items-center justify-center text-center p-8">
                                ...
                                <h3 className="font-black text-xl text-[#1A1A2E] uppercase">Select a Ticket</h3>
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#1A1A2E]/80 backdrop-blur-sm">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                                <h3 className="font-black text-[#1A1A2E] uppercase tracking-wider flex items-center gap-2"><LifeBuoy size={18} className="text-[#E94E3C]" /> Create Support Ticket</h3>
                                <button onClick={() => setIsCreateModalOpen(false)} className="text-gray-400 hover:text-[#1A1A2E]"><X size={20} /></button>
                            </div>
                            ...
                            <form onSubmit={submitTicket} className="p-6 space-y-5">
                                <div className="space-y-1.5">
                                    ...
                                    <select required value={data.category} onChange={e => setData('category', e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#1A1A2E] text-sm focus:ring-2 focus:ring-[#E94E3C] outline-none">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <input required type="text" value={data.subject} onChange={e => setData('subject', e.target.value)} placeholder="Short description of the issue" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-[#1A1A2E] focus:ring-2 focus:ring-[#E94E3C] outline-none" />
                                </div>

                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Detailed Message *</label>
                                    <textarea required rows="4" value={data.message} onChange={e => setData('message', e.target.value)} placeholder="Please provide details..." className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-medium text-[#1A1A2E] text-sm focus:ring-2 focus:ring-[#E94E3C] outline-none resize-none"></textarea>
                                </div>

                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Attachment (Optional)</label>
                                    <input type="file" onChange={e => setData('attachment', e.target.files[0])} accept=".jpg,.png,.pdf" className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-black file:uppercase file:bg-gray-100 file:text-[#1A1A2E] hover:file:bg-gray-200 cursor-pointer" />
                                </div>
