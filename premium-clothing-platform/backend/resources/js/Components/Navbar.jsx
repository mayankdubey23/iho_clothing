    ...
    return (
        <>
            ...
            {vertical && !admin && (
                ...
                >
                    <div className="flex h-full w-72 flex-col py-6">
                        <Link href="/" className="ml-[18px] grid size-12 place-items-center bg-[#282c3f] text-[10px] font-black tracking-widest text-white shadow-md transition-transform hover:scale-105" title={brandName}>
                        ...
                        <div className="pointer-events-none absolute left-0 top-1/2 flex w-20 -translate-y-1/2 justify-center transition-opacity duration-200 group-hover/sidebar:opacity-0">
                            <span className="-rotate-90 text-[9px] font-black uppercase tracking-[0.45em] text-[#282c3f]/40">Menu</span>
                        </div>
                        ...
                        <div className="flex flex-1 translate-x-4 flex-col opacity-0 transition-all duration-500 ease-out group-hover/sidebar:translate-x-0 group-hover/sidebar:opacity-100">
                            <div className="px-6 pb-8 pt-6">
                                <p className="text-xl font-black uppercase italic tracking-tighter text-[#282c3f]">{brandName}</p>
                                ...
                                {[...navLinks, { name: user ? 'Account' : 'Login', href: user ? '/account' : '/login' }, { name: 'Wishlist', href: '/wishlist' }].map((link, index) => {
                                    ...
                                    return (
                                        <Link
                                            ...
                                            href={link.href}
                                            className={`relative flex min-h-12 items-center rounded-xl px-5 text-xs font-black tracking-wider uppercase transition-all duration-300 ${isActive ? 'bg-[#ff3f6c] text-white shadow-[0_5px_15px_rgba(11,92,173,0.2)]' : link.sale ? 'text-red-500 hover:bg-slate-50' : 'text-slate-600 hover:bg-slate-50 hover:text-[#ff3f6c] hover:translate-x-1'}`}
                                        >
                                            {link.name}
                                            {link.name === 'Wishlist' && wishlistCount > 0 && <span className="ml-2 text-[10px] text-red-500 font-bold">({wishlistCount})</span>}
                                            {link.highlight && <span className="ml-auto rounded-md bg-[#282c3f] px-2 py-0.5 text-[8px] font-black uppercase tracking-widest text-white shadow-sm">New</span>}
                                        </Link>
                                    );
                                })}

                                <button onClick={() => setIsSearchOpen(!isSearchOpen)} className="flex min-h-12 items-center rounded-xl px-5 text-left text-xs font-black uppercase tracking-wider text-slate-600 transition-colors hover:bg-slate-50 hover:text-[#ff3f6c]">
                                    Search
                                </button>
                                <button onClick={() => setIsCartOpen(true)} className="flex min-h-12 items-center rounded-xl px-5 text-left text-xs font-black uppercase tracking-wider text-slate-600 transition-colors hover:bg-slate-50 hover:text-[#ff3f6c]">
                ...
                {vertical && isSearchOpen && (
                    ...
                    >
                        <form onSubmit={handleSearchSubmit} className="flex items-center gap-4">
                            <Search size={20} className="text-[#ff3f6c]" />
                            <input
                                ...
                                className="min-w-0 flex-1 border-none bg-transparent p-0 text-xs font-black uppercase tracking-widest text-[#282c3f] outline-none placeholder:text-slate-300 focus:ring-0"
                            />
                        ...
                        <div className="mt-5 flex flex-wrap gap-2 border-t border-slate-100 pt-5">
                            {trendingSearches.map((term) => (
                                <button key={term} type="button" onClick={() => { setIsSearchOpen(false); router.get('/shop', { search: term }); }} className="bg-slate-50 border border-slate-200 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-500 transition hover:bg-[#ff3f6c] hover:text-white rounded-lg">
                                    {term}
                                </button>
                            ))}
                        </div>
            ...
            <motion.header
                ...
                className={`fixed top-0 left-0 right-0 z-40 flex justify-center transition-all duration-500 px-0 sm:px-4 ${vertical ? 'lg:hidden' : ''}`}
            >
                <div className={`relative flex items-center justify-between w-full transition-all duration-500 bg-white/90 backdrop-blur-2xl border-b border-slate-100 ${isScrolled ? 'shadow-[0_10px_30px_rgba(30,41,59,0.05)] sm:rounded-none' : 'sm:rounded-lg'} px-5 py-2`}>
                    ...
                    <div className="flex-1 flex items-center gap-4 lg:hidden">
                        ...
                        <button onClick={() => setIsSearchOpen(!isSearchOpen)} className="text-[#282c3f] hover:text-[#ff3f6c] transition-colors p-1 active:scale-90">
                    ...
                    <div className="flex-shrink-0 flex items-center justify-center lg:justify-start lg:w-48 z-20">
                        <Link href={admin ? '/franchise-superadmin' : '/'} className="flex items-center gap-3 group">
                            <div className="grid size-8 place-items-center bg-[#282c3f] text-white text-[10px] font-black tracking-widest group-hover:bg-[#ff3f6c] transition-colors shadow-md">
                                {logoUrl ? <img src={logoUrl} alt={brandName} className="h-full w-full object-contain p-1" /> : logoMark}
                            </div>
                            <span className="hidden md:block text-xl font-black tracking-tighter uppercase leading-none text-[#282c3f] italic xl:text-2xl">
                                {brandName}
                            </span>
                        </Link>
                    </div>
                    ...
                    <nav className="hidden lg:flex flex-1 justify-center gap-5 xl:gap-7" onMouseLeave={() => setHoveredLink(null)}>
                        {navLinks.map((link, index) => (
                            <Link
                                key={`desktop-nav-${index}`} href={link.href} onMouseEnter={() => setHoveredLink(link.name)}
                                className={`relative whitespace-nowrap py-2 text-[10px] font-black uppercase tracking-[0.12em] transition-colors group xl:text-[11px] xl:tracking-[0.15em] ${link.sale ? 'text-red-500' : 'text-[#282c3f] hover:text-[#ff3f6c]'}`}
                            >
                                {link.name}
                                <motion.div
                                    className={`absolute bottom-0 left-0 h-[2px] ${link.sale ? 'bg-red-500' : 'bg-[#ff3f6c]'}`}
                                    initial={{ width: 0 }} animate={{ width: hoveredLink === link.name ? '100%' : 0 }} transition={{ duration: 0.3 }}
                                />
                                {link.highlight && <span className="absolute -top-2.5 -right-4 text-[7px] bg-[#282c3f] text-white px-1.5 py-0.5 font-black uppercase tracking-widest rounded-sm">NEW</span>}
                            </Link>
                        ))}
                    </nav>
                            ...
                            >
                                <div className="grid gap-8 md:grid-cols-[1fr_1fr_1fr_220px]">
                                        ...
                                        <div key={column.title}>
                                            ...
                                            <div className="flex flex-col gap-3">
                                                {column.links.map(([label, href]) => (
                                                    <Link key={href} href={href} className="text-xs font-bold uppercase tracking-widest text-slate-500 transition hover:text-[#282c3f] hover:translate-x-1">
                                                        {label}
                                                    </Link>
                                                ))}
                                            </div>
                                    ...
                                    <div className="bg-gradient-to-br from-blue-50 to-slate-100 p-5 rounded-xl border border-blue-100/50">
                                        ...
                                        <Link href={site.nav_promo_link || '/shop?sort=newest'} className="mt-5 inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#282c3f] hover:text-[#ff3f6c] transition-colors">
                    ...
                    <div className="flex-1 flex justify-end items-center gap-4 lg:gap-6 lg:w-48">
                        {!admin && (
                            <>
                                <button onClick={() => setIsSearchOpen(!isSearchOpen)} className="text-[#282c3f] hover:text-[#ff3f6c] transition-colors hidden lg:block p-1 active:scale-95">
                                    <Search size={20} strokeWidth={2} />
                                </button>
                                <Link href="/wishlist" className="relative text-[#282c3f] hover:text-[#ff3f6c] transition-colors hidden sm:block p-1 active:scale-95">
                                    ...
                                    {wishlistCount > 0 && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-1.5 -right-2 size-4 bg-[#ff3f6c] text-white text-[8px] font-black rounded-full flex items-center justify-center border border-white">{wishlistCount}</motion.span>}
                                </Link>
                            </>
                        )}
                        ...
                        {user ? (
                                        ...
                                        >
                                            ...
                                            <div className="p-2">
                                                {USER_MENU_ITEMS.map((item, i) => {
                                                    ...
                                                    return (
                                                        <Link key={`user-menu-${i}`} href={item.href} onClick={() => setIsUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-3 text-[10px] font-black uppercase tracking-[0.1em] text-slate-500 hover:text-[#ff3f6c] hover:bg-blue-50/50 rounded-xl transition-all">
                            ...
                            <Link href="/login" className="hidden sm:block text-[#282c3f] hover:text-[#ff3f6c] transition-colors p-1 active:scale-95"><User size={20} strokeWidth={2} /></Link>
                        )}
                        ...
                        {!admin && (
                            <button onClick={() => setIsCartOpen(true)} className="relative text-[#282c3f] hover:text-[#ff3f6c] transition-colors flex items-center p-1 active:scale-95">
                                ...
                                {cartCount > 0 && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -top-1.5 -right-2 size-4 bg-[#282c3f] text-white text-[8px] font-black rounded-full flex items-center justify-center border border-white shadow-sm">{cartCount}</motion.span>}
                            </button>
                        )}
                    </div>
                </div>
                        ...
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="absolute top-full left-0 w-full bg-white/95 backdrop-blur-xl border-b border-slate-100 shadow-xl overflow-hidden z-[40]">
                            <div className="max-w-4xl mx-auto px-6 py-6">
                                ...
                                <div className="mt-5 flex flex-wrap gap-2 border-t border-slate-100/50 pt-5">
                                    {trendingSearches.map((term) => (
                                        <button key={term} type="button" onClick={() => { setIsSearchOpen(false); router.get('/shop', { search: term }); }} className="bg-slate-50/50 border border-slate-200 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-500 transition hover:bg-[#ff3f6c] hover:text-white rounded-lg">
                                            {term}
                                        </button>
                                    ))}
                                </div>
