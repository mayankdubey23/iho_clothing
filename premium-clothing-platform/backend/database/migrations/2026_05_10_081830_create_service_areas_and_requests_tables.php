...
return new class extends Migration {
    public function up()
    {
        // 1. Approved Assigned Areas (Managed by Super Admin)
        if (!Schema::hasTable('franchise_service_areas')) {
            Schema::create('franchise_service_areas', function (Blueprint $table) {
                ...
                $table->string('delivery_coverage')->default('Standard'); // e.g. "5km radius" or "Full Pincode"
                $table->boolean('is_active')->default(true);
                $table->timestamps();
            });
        }

        // 2. Expansion / Change Requests (Raised by Franchise)
        if (!Schema::hasTable('service_area_requests')) {
